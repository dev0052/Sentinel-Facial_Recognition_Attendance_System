...
def send_mail_with_image(image_path):
    """Sends an email with the intruder image attached."""
    sender_email = "ubuntuvm22.alerts@gmail.com"
    sender_password = "toai rnud vpod yhnq"
    recipient_email = "dev.sharma2021@vitbhopal.ac.in"

    subject = "Intruder Alert"
    body = "Intruder detected. Please find the attached image for verification."
    ...
    try:
        ...
        with open(image_path, "rb") as img_file:
            ...
            msg.add_attachment(img_data, maintype="image", subtype="jpeg", filename=image_path.split("/")[-1])
...
EMAIL_PASSWORD = "lerk bjze xhcl hced"  # App password for the sender's email
        ...
        while recording_active:
            ...
            for (x, y, w, h) in faces:
                ...
                if conf < 50:
                    ...
                    cv2.imwrite(intruder_filename, img[y:y + h, x:x + w])
                    cv2.putText(img, "Intruder", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

                    # Send an email with the intruder image
...
def on_closing():
    """Handle window close event gracefully."""
    try:
        cam.release()  # Release the camera resource if active
    except NameError:
        pass  # Camera might not be initialized yet
    cv2.destroyAllWindows()
    cv2.waitKey(1)
    window.destroy()
