...
def save_pass():
    ...
    password_file = os.path.join(path, "psd.txt")

    if not os.path.exists(password_file):
        new_pass = tsd.askstring('New Password', 'Enter a new password', show='*')
        if new_pass:
            with open(password_file, 'w') as f:
                f.write(new_pass)
                mess.showinfo('Success', 'Password set successfully!')
    else:
...
def change_pass():
    ...
    tk.Button(master, text="Save", command=save_pass).place(x=150, y=160)
...
btn_track = tk.Button(window, text="Track Attendance", command=track_images)
